...
// We can't implement From/Into Option<Proto> because of the orphan rule, so we make our own traits that mirror them and implement those instead.
pub trait FromProto<P>: Sized {
    fn from_proto(p: P) -> Self;
}
pub trait TryFromProto<P>: Sized {
    type Error;
    fn try_from_proto(p: P) -> Result<Self, Self::Error>;
}
...
impl<A, B: FromProto<A>> IntoProto<B> for A {
    fn into_proto(self) -> B {
        B::from_proto(self)
    }
}
impl<A, B: TryFromProto<A>> TryIntoProto<B> for A {
    type Error = B::Error;
    fn try_into_proto(self) -> Result<B, Self::Error> {
        B::try_from_proto(self)
    }
}
