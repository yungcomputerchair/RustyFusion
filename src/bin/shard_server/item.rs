use rusty_fusion::{enums::eItemLocation, error::BadPayload};

use num_traits::FromPrimitive;

use super::*;

pub fn item_move(clients: &mut ClientMap, state: &mut ShardServerState) -> Result<()> {
    ...
    let location_from = eItemLocation::from_i32(pkt.eFrom).ok_or(BadPayload::build(
        client,
    ...
    let location_to = eItemLocation::from_i32(pkt.eTo)
        .ok_or(BadPayload::build(client, format!("Bad eTo {}", pkt.eTo)))?;
