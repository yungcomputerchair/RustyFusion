...
pub async fn trade_confirm(clients: &ClientMap<'_>, state: &mut ShardServerState) -> FFResult<()> {
    ...
    if let Ok((items, items_other)) = trade.resolve((&mut player, &mut player_other)) {
        ...
        *state.get_player_mut(pc_id_other).unwrap() = player_other.clone();
        ...
        client_other.send_packet(P_FE2CL_REP_PC_TRADE_CONFIRM_SUCC, &resp);
