...
mod commands {
    ...
    use rusty_fusion::{ai::AI, db_run_sync};
    ...
    fn cmd_ban(
        ...
        let acc_id = if is_ban_i {
            ...
            db_run_sync!(db => db.find_account_from_player(pc_uid))
                .unwrap()
                .id
        } else {
        ...
        match db_run_sync!(db =>
            db.ban_account(acc_id, banned_until, ban_reason_clone)
        ) {
            Ok(()) => {
                let ban_msg = format!(
                    "Account {} banned for {}\n\
